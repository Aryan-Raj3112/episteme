...
fun Page.applyReaderThemeForDisplayPolicy(
    isDarkTheme: Boolean,
    themeBackgroundColor: Color,
    themeTextColor: Color,
    transformNonMathSvgText: (String, Color) -> String = { svg, _ -> svg }
): Page {
    return copy(
        content = content.map {
            it.applyReaderThemeForDisplay(isDarkTheme, themeBackgroundColor, themeTextColor, transformNonMathSvgText)
        }
    )
}
...
private fun ContentBlock.applyReaderThemeForDisplay(
    ...
    themeBackgroundColor: Color,
    themeTextColor: Color,
    transformNonMathSvgText: (String, Color) -> String
): ContentBlock {
    ...
    return when (this) {
        ...
        is MathBlock -> copy(
            svgContent = if (isFromMathJax) {
                ...
                svgContent?.let { transformNonMathSvgText(it, themeTextColor) }
            },
            style = themedStyle
        )
