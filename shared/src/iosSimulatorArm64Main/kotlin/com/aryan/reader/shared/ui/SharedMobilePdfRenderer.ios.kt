...
internal actual fun rememberSharedMobilePdfPageThumbnail(
    ...
    var thumbnail by remember(book.path, pageIndex, password) {
        mutableStateOf<SharedMobilePdfPageThumbnail?>(IosPdfThumbnailCache.get(book.path, pageIndex, password))
    }

    LaunchedEffect(book.path, pageIndex, password) {
        if (thumbnail == null) {
            thumbnail = IosPdfThumbnailCache.get(book.path, pageIndex, password)
                ?: IosPdfiumRenderer.renderThumbnail(book.path, pageIndex, password)?.also { rendered ->
                    IosPdfThumbnailCache.put(book.path, pageIndex, password, rendered)
                }
                ?: SharedMobilePdfPageThumbnail()
        }
    }
...
private object IosPdfThumbnailCache {
    ...
    fun get(path: String?, pageIndex: Int, password: String?): SharedMobilePdfPageThumbnail? =
        entries[key(path, pageIndex, password)]

    fun put(path: String?, pageIndex: Int, password: String?, thumbnail: SharedMobilePdfPageThumbnail) {
        val key = key(path, pageIndex, password)
...
private object IosPdfiumRenderer {
    ...
    suspend fun renderThumbnail(path: String?, pageIndex: Int, password: String?): SharedMobilePdfPageThumbnail? =
        IosPdfiumRuntime.mutex.withLock {
