...
Result (2026-08-11): complete. The root `verifyCodebaseArchitecture` task now runs from normal project `check` tasks and is configuration-cache-safe. It caps every new production Kotlin file at 1,500 lines, gives each of the 32 existing larger files an exact non-growth ceiling, prevents `Common.kt` from returning, rejects Android dependencies on iOS/desktop implementations, and duplicates the portable shared-source boundary at the repository level. The existing shared portable-source task remains attached to shared verification. No broad formatter or static-analysis rollout was added because applying a new global style baseline during behavior-preserving modularization would mix mechanical churn with architectural evidence.

The completion audit also repaired three test-infrastructure defects exposed only by full-suite execution: shared desktop tests now include the current-OS Compose native runtime, the desktop toolbar test correctly distinguishes the mobile `AUTO_SCROLL` model from desktop quick actions, and two Android tests no longer depend on the deleted `Common.kt` path or invoke a mocked URI while verifying a zero-call contract.

Final verification: the architecture gate passes twice with configuration-cache reuse; all 982 shared desktop tests pass; Android OSS and Pro production compilation, Android instrumented-test compilation, and iOS simulator compilation pass. The Android unit suite has 699 passing tests and two pre-existing localization-coverage failures: Vietnamese and Estonian each lack the same 139 recently added unified-library/listen/audiobook strings. English fallback remains functional, but supplying reviewed translations is intentionally not disguised as code cleanup or filled with machine-copied English. This is the only known verification exception. `git diff --check` and the `.gitignore` invariant pass, and no emulator was run.

Final structural metrics: `Common.kt` is absent; the largest Phase 5 replacement is 669 LOC; portable shared Android/JVM imports are zero; new production Kotlin files are limited to 1,500 LOC; and all larger legacy files are ratcheted. Across Phases 1–6, the recorded production Kotlin change is Android +839 LOC and shared +1,233 LOC. Those additions are primarily explicit imports, file boundaries, and shared ownership seams; the cleanup optimized ownership and change isolation rather than treating raw deletion as the goal.
...
| 5 | Complete (2026-08-11) | +290 | 0 | `a5aeda2f` | Deleted `Common.kt`, regrouped its implementation into nine feature files, removed ambiguous generic aliases and six identity wrappers, and passed focused tests plus OSS/Pro and Android-test compilation. |
| 6 | Complete (2026-08-11) | -9 | 0 | this phase commit | Added cache-safe size/dependency/dumping-ground ratchets, removed stale split-source labels, repaired full-suite test infrastructure, and completed the cross-platform audit. The only exception is reviewed localization work for 139 Vietnamese and Estonian strings. |
